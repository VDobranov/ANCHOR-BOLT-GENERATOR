...
- Доступные типы болтов для выбора: 1 (исполнения 1, 2), 2 (исполнение 1), 4;
...
- **Стандартные атрибуты типов**:
  - `NominalDiameter` (диаметр резьбы) — определяется один раз на уровне типа
  - `NominalLength` (номинальная длина) — определяется один раз на уровне типа
...
- **Property Sets (PSets)**: Определяются в типах, применяются ко всем инстансам через отношение IfcRelDefinesByType:
  ...
  - Pset_MechanicalFastenerAnchorBolt (для сборки):
    ...
    - AnchorBoltProtrusionLength - не заполняется!
