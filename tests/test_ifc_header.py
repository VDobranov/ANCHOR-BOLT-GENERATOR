...
class TestIFCHeader:
    ...
    def test_file_description_exists(self, ifc_doc):
        ...
        assert (
            "implementation_level" in parsed
        ), "Отсутствует implementation_level в FILE_DESCRIPTION"
    ...
    def test_file_description_implementation_level(self, ifc_doc):
        ...
        assert (
            parsed["implementation_level"] == "2;1"
    ...
    def test_file_name_not_empty(self, ifc_doc):
        ...
        assert parsed["name"] and parsed["name"].strip(), "FILE_NAME.name не должен быть пустым"
    ...
    def test_file_name_organization_not_software_company(self, ifc_doc):
        ...
        assert org != originating, "organization не должно содержать имя компании-разработчика ПО"
    ...
    def test_file_name_originating_system_not_placeholder(self, ifc_doc):
        ...
        assert (
            originating not in invalid_values
        ), f"originating_system не должно быть '{originating}'"

        # Не должно содержать только имя ОС
        assert originating.lower() != "windows", "originating_system не должно быть названием ОС"
